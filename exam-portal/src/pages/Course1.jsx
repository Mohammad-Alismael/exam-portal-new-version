...
function Course1(props) {
  ...
  const postAnnouncement = (e) => {
    axios
      .post("http://localhost:8080/set-announcement-to-students", {
  ...
  const loadAnnouncements = async () => {
    ...
    const Announcements = await axios.get(
      `http://localhost:8080/get-announcements?id=${props.user.user_id}`
    );
    setAnnouncements(Announcements.data);
  };
  const loadAnnouncementsForStudents = async () => {
    const Announcements = await axios.post(
      "http://localhost:8080/get-announcement-student-id",
      {
    ...
    Announcements.data.map((val, index) => {
      ...
      setAnnouncements([
        ...announcements,
        { instructorId: val[0], announcementText: val[1], createdAt: val[2] },
      ]);
    });
  };
  const getClassroom = async () => {
    const promise = new Promise((resolve, reject) => {
      axios
        .post("http://localhost:8080/get-class-students", {
          instructorId: props.user.user_id,
        })
        .then((data) => {
          resolve(data.data);
  ...
  const getClassRoomForStudents = async () => {
    const classroom1 = await axios.post(
      "http://localhost:8080/get-instructor-id-from-student-id",
      {
        studentId: props.user.user_id,
      }
    );
    ...
    const classroomStudents = await axios.post(
      "http://localhost:8080/get-class-students",
      {
        instructorId,
      }
    );
  ...
  const getExamsList = async () => {
    ...
    const promise = new Promise((resolve, reject) => {
      axios
        .post("http://localhost:8080/list-all-exams-creator-id", {
  ...
  const getExamListForStudents = async () => {
    const examss = await axios.post(
      "http://localhost:8080/get-exam-id-student-id",
      {
        studentId: props.user.user_id,
      }
    );
              ...
              <TabPanel value='3'>
                <Paper fullwidth elevation={0}>
                  ...
                  <Grid
                    ...
                    justifyContent='center'
                  >
                    <Typography variant='h2'>Classmates</Typography>
