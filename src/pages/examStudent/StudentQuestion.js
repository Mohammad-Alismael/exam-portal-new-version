...
function StudentQuestion() {
  ...
  const { questions, examDetails, questionIndex } = useSelector(
    (state) => state.ExamStudentReducer
  );
  ...
  useEffect(() => {
    let html = convertToHTML(
      calcState(questions[questionIndex].questionText).getCurrentContent()
    );
          ...
          {questions[questionIndex]["previewFile"] != null ? (
            <Grid xs={12} item>
              <img
                ...
                src={questions[questionIndex]["previewFile"]["preview"]}
                alt={"question img"}
              />
            </Grid>
          ) : null}
...
export default memo(StudentQuestion);
